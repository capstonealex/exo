...
void Joint::updateJoint(){
    ...
    //// For now any joint just gets value from motor 2
    ////qread = CO_OD_RAM.actualMotorPositions.motor2
    /// q = readMotorToAngle(qread)
    q = CO_OD_RAM.actualMotorPositions.motor2;
//  test  cout<<"Joint id number "<<id << "read in value:" << qread " Thinks it's @ pos "<<q<<"\n";
}